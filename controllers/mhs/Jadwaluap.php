...
class Jadwaluap extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['title'] = 'UAP (Ujian Akhir Program)';
		$data['judul'] = 'UAP Mahasiswa';
		//setting krs
		$data['setting_krs'] = $this->db->get('set_krs')->row_array();
		$data['setting'] = $this->db->get('set_krs')->row_array();
		$data['jadwal'] = $this->JadwaluapModel->getDataUapSW('jadwal_uap')->result();
	...
	public function print_uap($data)
	{
		...
		$getUap = $this->JadwaluapModel->getDataUapSW('jadwal_uap')->result();
		...
		foreach ($getUap as $d) {
			...
			$pdf->SetFont('Times', '', 9);
            ob_end_clean();
		    $pdf->Output( 'Kartu UAP STIKes Bogor Husada'.'_'. $mhs['nama_mhs'].'.pdf','D');
	}
