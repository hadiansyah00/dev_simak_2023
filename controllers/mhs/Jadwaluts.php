...
class Jadwaluts extends CI_Controller
{
	...
	public function index()
	{
		...
		$data['getUts'] = $this->JadwalutsModel->getMatkul_KRS($mhs['kd_jurusan'], $ta['id_ta']);
		...
		$this->load->view('mhs/jadwal_uts', $data);
		$this->load->view('mhs/templates/footer');
	}
	
	public function print_kelas_pagi($data)
	{
		...
		$getUts = $this->JadwalutsModel->getMatkul_KRS($mhs['kd_jurusan'], $ta['id_ta']);
		...
		$pdf->Output( 'Kartu UTS STIKes Bogor Husada'.'_'. $mhs['nama_mhs'].'.pdf','D');
	}
	
		public function print_kelas_karyawan($data)
	{
		$data['title'] = 'Kartu UTS';
		$data['judul'] = 'Kartu UTS Mahasiswa';
        $mhs = $this->KrsModel->getDataMhs();
